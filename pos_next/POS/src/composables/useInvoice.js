...
export function useInvoice() {
	...
	function addItem(item, quantity = 1, merge = true) {
		let existingItem = null

		if (merge) {
			existingItem = invoiceItems.value.find(
	...
	function removeItem(identifier) {
		const itemToRemove = invoiceItems.value.find(
			(i) => i.row_id === identifier || i.item_code === identifier,
		)
	...
	function updateItemRate(identifier, rate) {
		const item = invoiceItems.value.find(
			(i) => i.row_id === identifier || i.item_code === identifier,
		)
		if (item) {
	...
	function updateItemDiscount(identifier, discountPercentage) {
		const item = invoiceItems.value.find(
			(i) => i.row_id === identifier || i.item_code === identifier,
		)
		if (item) {
	...
	async function saveDraft() {
		...
		const invoiceData = {
			...
			items: rawItems.map((item) => {
				// Build description with comment if exists
				let itemDescription = item.item_name || item.item_code
				if (item.comment) {
					itemDescription = `${item.item_name || item.item_code}\n📝 ${item.comment}`
				}
				return {
				item_code: item.item_code,
				item_name: item.item_name,
	...
	async function submitInvoice() {
		...
		try {
			...
			// Get customer - use selected customer or fallback to profile customer
			let customerValue = customer.value?.name || customer.value
			
			// If no customer selected, try to get default customer from POS Profile
			if (!customerValue && posProfile.value) {
				try {
					const defaultCustomerResult = await getDefaultCustomerResource.submit({
						pos_profile: posProfile.value,
					})
					if (defaultCustomerResult && defaultCustomerResult.customer) {
						customerValue = defaultCustomerResult.customer
					}
				} catch (error) {
					// Silently fail - customer is now optional
					console.log("No default customer available in POS Profile")
				}
			}
