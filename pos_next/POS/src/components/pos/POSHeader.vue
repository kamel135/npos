<template>
	<div
		class="bg-white shadow-sm sticky top-0 z-[200] transition-all duration-300"
		:style="isCollapsed ? { maxHeight: '0px', opacity: 0, padding: 0, margin: 0, overflow: 'hidden' } : { maxHeight: '100px', opacity: 1, overflow: 'visible' }"
	>
		<div class="flex items-center">
			<!-- Toggle Button - Always visible -->
			<div class="w-10 flex-shrink-0 flex items-center justify-center px-1">
				<button
					@click="toggleCollapse"
					class="w-7 h-7 flex items-center justify-center rounded-md hover:bg-gray-100 active:bg-gray-200 transition-colors touch-manipulation"
					:title="isCollapsed ? __('Show Header Info') : __('Hide Header Info')"
					:aria-label="isCollapsed ? __('Show Header Info') : __('Hide Header Info')"
				>
					<svg
			...
			<div class="w-16 flex-shrink-0 flex items-center justify-center">
				<button
					...
					:aria-label="'INTouch POS'"
					:title="__('INTouch POS')"
				>
			...
			<div class="flex-1 flex justify-between items-center gap-1 sm:gap-2 px-2 sm:px-4 md:px-6">
				...
				<div class="flex items-center gap-1 sm:gap-4 min-w-0 flex-1" :class="{ 'overflow-hidden': isCollapsed }">
					<div v-if="!isCollapsed" class="min-w-0 flex-shrink overflow-hidden">
						<div class="flex items-center gap-1 sm:gap-2">
							<h1 class="text-xs sm:text-base font-bold text-gray-900 truncate flex-shrink">{{ 'INTouch' }}</h1>
					...
					<!-- Time and Shift Duration - Hidden when collapsed -->
					<div v-if="!isCollapsed" class="hidden lg:flex items-center gap-4 ms-6 flex-shrink-0">
						<!-- Current Time -->
						<StatusBadge
							variant="blue"
							size="sm"
					...
					<!-- Mobile Time Display - Hidden when collapsed -->
					<div v-if="!isCollapsed" class="flex lg:hidden items-center text-[10px] text-gray-600 font-medium flex-shrink-0 ms-1">
				...
				<div class="flex items-center gap-0.5 sm:gap-1 md:gap-2 flex-shrink-0">
					...
					<!-- Divider - Hidden when collapsed or on mobile -->
					<div v-if="!isCollapsed" class="w-px h-4 sm:h-6 bg-gray-200 hidden md:block"></div>

					<!-- Language Switcher - Always visible on desktop, hidden on mobile -->
					<div class="hidden md:block">
						<LanguageSwitcher />
					</div>
...
function toggleCollapse() {
	...
	if (typeof window !== 'undefined') {
		localStorage.setItem('pos-header-collapsed', isCollapsed.value.toString())
	}
	// Emit event to parent to adjust layout
	emit('header-collapsed', isCollapsed.value)
}

// Expose toggleCollapse for parent component to call
defineExpose({
	toggleCollapse
})
