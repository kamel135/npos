...
def after_install():
	...
	try:
		...
		install_fixtures()
		# Force update print format to ensure Chrome fixes are applied
...
def after_migrate():
	...
	try:
		...
		# Force update print format HTML to ensure Chrome fixes are always applied
		ensure_print_format_updated(quiet=True)
...
def install_print_format(doc_dict, quiet=False):
	...
	Always updates HTML to ensure latest Chrome fixes are applied
	...
	try:
		...
		if frappe.db.exists("Print Format", name):
			...
			# Always update HTML to ensure latest fixes are applied
			# This is important for Chrome print compatibility fixes
			# Force update HTML every time to prevent stale data
			html_changed = False
			if "html" in doc_dict:
				new_html = doc_dict.get("html", "").strip()
				existing_html = existing_doc.get("html", "").strip()
				# Always update to ensure we have the latest from fixtures
				existing_doc.set("html", new_html)
				html_changed = True  # Always true since we always update

			# Update other key properties
			update_fields = ["doc_type", "module", "disabled", "standard", "custom_format", "print_format_type"]
			other_updated = False
			...
			# Always save HTML to ensure Chrome fixes are always applied
			# Always save since html_changed is always True (we force update)
			if html_changed or other_updated:
				...
				frappe.clear_cache()
				if not quiet:
					log_message(f"Updated Print Format HTML: {name} (Chrome fixes applied)", level="info", indent=1)
...
def ensure_print_format_updated(quiet=False):
	"""
	Force update print format HTML from fixtures to ensure latest fixes are applied
	This is especially important for Chrome print compatibility fixes
	...
	try:
		...
		print_format_file = os.path.join(
			frappe.get_app_path("pos_next", "fixtures"), 
			"print_format.json"
		)
		...
		for pf in print_formats:
			if pf.get("name") == "POS Next Receipt":
				install_print_format(pf, quiet=quiet)
				break
				
	except Exception as e:
		if not quiet:
			log_message(f"Error ensuring print format is updated: {str(e)}", level="error")
		frappe.log_error(
			title="Print Format Update Error",
			message=frappe.get_traceback()
		)
