...
def remove_custom_shift_invoice_number(doc):
    """
    Remove custom_shift_invoice_number field from document to prevent database errors.
    This field may be sent from frontend but doesn't exist in the database schema.
    """
    field_name = 'custom_shift_invoice_number'
    ...
    try:
        if field_name in doc.__dict__:
            del doc.__dict__[field_name]
    except (KeyError, AttributeError):
        pass
    
    try:
        if hasattr(doc, '_doc') and isinstance(doc._doc, dict) and field_name in doc._doc:
            del doc._doc[field_name]
    except (KeyError, AttributeError):
        pass
    
    try:
        if hasattr(doc, '_changed') and field_name in doc._changed:
            doc._changed.discard(field_name)
    except (KeyError, AttributeError):
        pass
    
    # Also remove from flags if present
    try:
        if hasattr(doc, 'flags'):
            if hasattr(doc.flags, field_name):
                delattr(doc.flags, field_name)
            if isinstance(doc.flags, dict) and field_name in doc.flags:
                del doc.flags[field_name]
    except (KeyError, AttributeError):
        pass
    
    # Remove from meta's valid columns cache if it exists
    try:
        if hasattr(doc, 'meta') and hasattr(doc.meta, '_valid_columns'):
            if field_name in doc.meta._valid_columns:
                doc.meta._valid_columns = [c for c in doc.meta._valid_columns if c != field_name]
...
def update_invoice(data):
    ...
    try:
        ...
        # Remove any custom fields that don't exist in the doctype
        # This prevents errors when trying to save non-existent fields
        if isinstance(data, dict):
            # Get the meta to check which fields actually exist
            try:
                meta = frappe.get_meta(doctype)
                valid_fields = set(meta.get_fieldnames_with_value())
                
                # Remove any fields that don't exist in the doctype
                # This includes custom fields that haven't been created yet
                fields_to_remove = []
                for field in data.keys():
                    if field not in valid_fields and (field.startswith('custom_') or field == 'custom_shift_invoice_number'):
                        fields_to_remove.append(field)
                
                for field in fields_to_remove:
                    data.pop(field, None)
            except Exception:
                # If meta check fails, just remove known problematic fields
                fields_to_remove = ['custom_shift_invoice_number']
                for field in fields_to_remove:
                    data.pop(field, None)
        ...
        # Explicitly remove custom_shift_invoice_number from document if it exists
        # This prevents Frappe from trying to save it
        remove_custom_shift_invoice_number(invoice_doc)
        ...
        # Ensure customer exists - use POS Profile customer as fallback if no customer selected
        customer_name = invoice_doc.get("customer")
        
        # If no customer selected, try to use POS Profile default customer
        if not customer_name and pos_profile_doc:
            if hasattr(pos_profile_doc, "customer") and pos_profile_doc.customer:
                customer_name = pos_profile_doc.customer
                invoice_doc.customer = customer_name
                # Fetch customer_name field
                try:
                    customer_doc = frappe.get_doc("Customer", customer_name)
                    invoice_doc.customer_name = customer_doc.customer_name
                except Exception:
                    pass  # Will be set automatically by ERPNext
        
        # If still no customer, create or use a default "Walk-in Customer"
        if not customer_name:
            default_customer_name = "Walk-in Customer"
            # Check if Walk-in Customer exists, if not create it
            if not frappe.db.exists("Customer", default_customer_name):
                try:
                    ...
                    walk_in_customer.flags.ignore_permissions = True
                    walk_in_customer.insert()
                    customer_name = walk_in_customer.name
                except Exception as e:
                    frappe.log_error(f"Failed to create walk-in customer: {e}")
                    # Try to find any customer as last resort
                    all_customers = frappe.get_all("Customer", limit=1, order_by="creation desc")
                    if all_customers:
        ...
        # Ensure posting_date is set BEFORE any calculations or validations
        # (required for Folio Entry and other integrations that validate during set_missing_values)
        posting_date_value = invoice_doc.get("posting_date") or nowdate()
        posting_time_value = invoice_doc.get("posting_time") or nowtime()
        
        invoice_doc.posting_date = posting_date_value
        invoice_doc.posting_time = posting_time_value
        
        # Set posting_date for all Folio Entry child table entries (if they exist)
        if hasattr(invoice_doc, "folio_entries") and invoice_doc.folio_entries:
            for folio_entry in invoice_doc.folio_entries:
                if not folio_entry.get("posting_date"):
                    folio_entry.posting_date = posting_date_value
        ...
        # ========================================================================
        for item in invoice_doc.get("items", []):
            # Ensure child table fields are set correctly
            if not item.get("parent"):
                item.parent = invoice_doc.name
            if not item.get("parenttype"):
                item.parenttype = "Sales Invoice"
            if not item.get("parentfield"):
        ...
        # Ensure posting_date is set before save (final check for Folio Entry)
        if not invoice_doc.get("posting_date"):
            invoice_doc.posting_date = nowdate()
        if not invoice_doc.get("posting_time"):
            invoice_doc.posting_time = nowtime()

        # Final removal of custom_shift_invoice_number right before save
        # This ensures it's completely removed from all document locations
        remove_custom_shift_invoice_number(invoice_doc)
        ...
        # Final check: remove field one more time right before save
        remove_custom_shift_invoice_number(invoice_doc)
        
        # Override get_valid_dict to exclude custom_shift_invoice_number
        original_get_valid_dict = invoice_doc.get_valid_dict
        def safe_get_valid_dict(*args, **kwargs):
            result = original_get_valid_dict(*args, **kwargs)
            result.pop('custom_shift_invoice_number', None)
            return result
        invoice_doc.get_valid_dict = safe_get_valid_dict
        
        try:
            invoice_doc.save()
        finally:
            # Restore original method
            invoice_doc.get_valid_dict = original_get_valid_dict
...
def submit_invoice(invoice=None, data=None):
    ...
    try:
        ...
        # Remove any custom fields that don't exist in the doctype before update
        # This prevents errors when trying to save non-existent fields
        invoice_clean = invoice.copy()
        if isinstance(invoice_clean, dict):
            # Get the meta to check which fields actually exist
            try:
                meta = frappe.get_meta(doctype)
                valid_fields = set(meta.get_fieldnames_with_value())
                ...
                # Remove any fields that don't exist in the doctype
                fields_to_remove = []
                for field in invoice_clean.keys():
                    if field not in valid_fields and (field.startswith('custom_') or field == 'custom_shift_invoice_number'):
                        fields_to_remove.append(field)
                
                for field in fields_to_remove:
                    invoice_clean.pop(field, None)
            except Exception:
                # If meta check fails, just remove known problematic fields
                fields_to_remove = ['custom_shift_invoice_number']
                for field in fields_to_remove:
        ...
        # Calculate shift invoice number BEFORE creating/updating invoice
        # This ensures we count correctly (before the new invoice is created)
        shift_invoice_number = None
        if pos_opening_shift:
            try:
                # Count existing invoices for this shift (excluding cancelled)
                # Exclude the current invoice if it already exists
                filters = {
                ...
                existing_invoices = frappe.db.count("Sales Invoice", filters=filters)
                
                # New invoice number is count + 1 (first invoice will be 1)
                shift_invoice_number = existing_invoices + 1
                
            except Exception as e:
                # If error occurs, log and continue without numbering
                frappe.log_error(f"Error calculating shift invoice number: {str(e)}", "Shift Invoice Numbering")

        # Get or create invoice
        if not invoice_name or not frappe.db.exists(doctype, invoice_name):
            created = update_invoice(json.dumps(invoice_clean))
            invoice_name = created.get("name")
            invoice_doc = frappe.get_doc(doctype, invoice_name)
        else:
            invoice_doc = frappe.get_doc(doctype, invoice_name)
            invoice_doc.update(invoice_clean)
        
        # Explicitly remove custom_shift_invoice_number from document if it exists
        # This prevents Frappe from trying to save it
        remove_custom_shift_invoice_number(invoice_doc)
        
        # Ensure customer is set - use POS Profile customer as fallback if no customer
        if not invoice_doc.get("customer"):
            customer_name = None
            
            # Try POS Profile customer first
            if pos_profile:
                try:
                    pos_profile_doc = frappe.get_cached_doc("POS Profile", pos_profile)
                    if hasattr(pos_profile_doc, "customer") and pos_profile_doc.customer:
                    ...
                    pass  # POS Profile customer is optional
            
            # If still no customer, create or use default "Walk-in Customer"
            if not customer_name:
                default_customer_name = "Walk-in Customer"
                if not frappe.db.exists("Customer", default_customer_name):
                    try:
        ...
        # Ensure posting_date is set (required for Folio Entry and other integrations)
        posting_date_value = invoice_doc.get("posting_date") or nowdate()
        posting_time_value = invoice_doc.get("posting_time") or nowtime()
        ...
        # Ensure all child table items have parent, parenttype, and parentfield set
        for item in invoice_doc.get("items", []):
            if not item.get("parent"):
                item.parent = invoice_doc.name
            if not item.get("parenttype"):
        ...
        # Store shift invoice number in remarks field after save (if calculated)
        if pos_opening_shift and shift_invoice_number:
            try:
                # Use remarks field to store shift invoice number (can be displayed in print format)
                current_remarks = invoice_doc.get("remarks") or ""
                shift_info = f"Shift Invoice #{shift_invoice_number}"
                if current_remarks and shift_info not in current_remarks:
                    invoice_doc.remarks = f"{current_remarks}\n{shift_info}"
                elif not current_remarks:
                    invoice_doc.remarks = shift_info
                # Ensure custom_shift_invoice_number is removed before final save
                remove_custom_shift_invoice_number(invoice_doc)
                invoice_doc.save()
            except Exception as e:
                # Continue if setting fails - not critical
                frappe.log_error(f"Error storing shift invoice number: {str(e)}", "Shift Invoice Numbering")
...
# ==========================================
# Fast Network Printing (CUPS) - Silent Printing
# ==========================================

@frappe.whitelist()
def fast_print_invoice(
    invoice_name,
    printer_setting=None,
    print_format=None,
):
    """
    Fast network printing using CUPS - no dialog, prints directly to printer
    This is much faster than window.print() method
    
    Args:
        invoice_name: Name of the Sales Invoice to print
        printer_setting: Name of Network Printer Settings (optional - will use default if available)
        print_format: Print format name (optional - will use invoice's POS Profile format)
    """
    try:
        # Get invoice to find POS Profile and print format
        ...
        if not print_format and invoice_doc.pos_profile:
            ...
            print_format = pos_profile_doc.get("print_format") or "POS Next Receipt"
        
        if not print_format:
            print_format = "POS Next Receipt"
        
        # Get printer setting - try to find from POS Profile or use provided
        if not printer_setting and invoice_doc.pos_profile:
            # Try to find network printer from POS Profile printer settings
            try:
                pos_profile_doc = frappe.get_doc("POS Profile", invoice_doc.pos_profile)
                # Check if POS Profile has printer_settings table (URY extension)
                if hasattr(pos_profile_doc, 'printer_settings') and pos_profile_doc.printer_settings:
                    for ps in pos_profile_doc.printer_settings:
                        if hasattr(ps, 'bill') and ps.bill == 1 and hasattr(ps, 'printer') and ps.printer:
                            printer_setting = ps.printer
                            break
                # Also check for custom fields that might have printer
                if not printer_setting and hasattr(pos_profile_doc, 'custom_table_order_printer') and pos_profile_doc.custom_table_order_printer:
                    printer_setting = pos_profile_doc.custom_table_order_printer
            except Exception:
                pass
        
        # If still no printer_setting, check for default network printer in localStorage equivalent
        # or use Frappe's print_by_server with default printer
        if printer_setting:
            try:
                # Use Frappe's built-in network printing
                ...
                return {
                    "success": False,
                    "error": "Network printing requires pycups. Install it with: pip install pycups"
                }
            except Exception as e:
                frappe.log_error(frappe.get_traceback(), "Fast Print Error")
                return {"success": False, "error": str(e)}
        else:
            # No network printer configured - fallback to regular print
            return {
                "success": False,
                "error": "No network printer configured. Please configure Network Printer Settings in POS Profile.",
                "fallback": True
            }
            
    except Exception as e:
        frappe.log_error(frappe.get_traceback(), "Fast Print Invoice Error")
        return {"success": False, "error": str(e)}
